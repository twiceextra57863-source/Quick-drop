...
public class QuickChestConfig {
    ...
    private static int actionsPerClick        = 1; // naya
    ...
    public static void setActionsPerClick(int v) {
        actionsPerClick = Math.max(1, Math.min(50, v)); }
    ...
    public static void load() {
        ...
        try (InputStream in = Files.newInputStream(CONFIG_PATH)) {
            ...
            autoMode            = Boolean.parseBoolean(
                p.getProperty("autoMode", "false"));
            autoCycles          = Integer.parseInt(
                p.getProperty("autoCycles", "8"));
            autoSpeedTicks      = Integer.parseInt(
                p.getProperty("autoSpeedTicks", "3"));
            returnDelayTicks    = Integer.parseInt(
                p.getProperty("returnDelayTicks", "10"));
            maxClicksPerSession = Integer.parseInt(
                p.getProperty("maxClicksPerSession", "5"));
            actionsPerClick     = Integer.parseInt(
                p.getProperty("actionsPerClick", "1"));
    ...
    public static void save() {
        try {
            Properties p = new Properties();
            p.setProperty("enabled",
                String.valueOf(enabled));
            p.setProperty("autoMode",
                String.valueOf(autoMode));
            p.setProperty("autoCycles",
                String.valueOf(autoCycles));
            p.setProperty("autoSpeedTicks",
                String.valueOf(autoSpeedTicks));
            p.setProperty("returnDelayTicks",
                String.valueOf(returnDelayTicks));
            p.setProperty("maxClicksPerSession",
                String.valueOf(maxClicksPerSession));
            p.setProperty("actionsPerClick",
                String.valueOf(actionsPerClick));
